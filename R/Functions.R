...
#' @param res The desired ouput resolution, in units of \code{region}'s projection. 
#' Ignored if \code{z_fix} is provided or if \code{region} is of class SpatRaster
#' or Raster*
...
#' @param terra ext
#' @param terra buffer
#' @param terra intersect
#' @param terra merge
#' @param terra mosaic
#' @param terra as.polygons
#' @param terra project
#' @param terra crs<-
#' @param terra crop
#' @param terra mask
...
importMap <- function(region, polys,
                      tile_id = 'TILEID', z_fix = NULL, res = NULL,
                      neighbor_distance = 5, FUN = NULL, mask = TRUE,
                      vals = 'location',
                      dir = tempdir(), ...){
  ...
  if (is.null(z_fix) & !methods::is(region,'SpatRaster')){
    z_fix <- fix_z(crs(region), res = res)
  }
  if (methods::is(region,'SpatRaster')){
    if (is.null(z_fix)){
      z_fix <- region
  ...
  tiles <- unlist(tiles)
  getMap(tiles, polys, tile_id = 'TILEID', dir = dir, ...)
  ...
  if (!all.equal(crs(z_fix), crs(region))){
...
makeWorld <- function(tiles,polys,cut_slope,tile_id = 'TILEID', proj = crs(polys),
  ...
  for (i in tiles){
    if (!file.exists(normalizePath(paste0(rd,"/",i,".fst"),mustWork=FALSE))){
      ...
      dem <- importMap(mzip, polys = polys, tile_id = tile_id, vals = vals,
                       z_fix = z_fix, 
                       dir = normalizePath(paste0(dir,"/Elevations/")))
