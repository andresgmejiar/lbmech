...
# That'll happen in the getMap function.
  ...
  filepath <- dem@file@name
  if (!is.null(crs)){
    dem <- extent(dem)
    dem <- as(dem,"SpatialPolygons")
    dem <- spTransform(dem,crs)
  }
  ...
  polys <- apply(cS[,.(left,right,bottom,top)],
                 1, FUN = function(x) as(extent(x), "SpatialPolygons"))
...
#' @param dem A character string or a RasterLayer object. Optional if the
...
getMap <- function(tiles, polys, tile_id = "TILEID", dem = "location",
                   dir = tempdir()){
  ...
  for (tile_name in tiles){
    if (!file.exists(paste0(rd,tile_name,".zip"))){
  ...
  if (!is.null(down)){
    ...
    # download the file
    if ((class(dem) != "RasterLayer") & is.null(polys$makeGrid)){
      for (i in 1:length(down)){
        tile_name <- tile_id[down[i]]
        file_path <- paste0(rd,tile_name,".zip")
        print(paste0("Downloading Tile ",tile_name," (",
                     i+1," of ",length(down),")"))
        download.file(url=polys@data[down[i],dem],
                      destfile=file_path)
      }
      ...
      for (i in 1:length(down)){
        ...
        clip <- crop(dem,polys[poly,])
        writeRaster(clip, filename = paste0(file_path,".tif"))
        zip(zipfile = paste0(file_path,".zip"),
            files = paste0(file_path,".tif"))
...
#' @param dem A character string or a RasterLayer object. Required only if the
...
getVelocity <- function(data, x, y, z, dt, ID, tau = NULL, tau_vmax = 0.995,
                        tau_nlrq = 0.995,k_init = 3.5, alpha_init = -0.1,
                        tile_id = "TILEID", dem = "location",
                        dir = tempdir()){
  ...
  # If z is a a raster layer, its simply extracting the z points from xy locations
  if (class(z) == "RasterLayer"){
    dem <- z
  ...
  } else if (class(z) == "SpatialPolygonsDataFrame"){
  ...
  # Make sure that the necessary files exist
    getMap(tiles=tiles,polys=z,tile_id=tile_id,dem=dem,dir=dir)
  ...
  # to collect all and consolidate
    ...
    for (tile in tiles){
      ...
      unzip(paste0(rd,tile,".zip"), exdir = str_remove(rd,"\\\\$"),junkpaths=TRUE)
      elevs <- raster(paste0(rd,tile,".tif"))
      ...
      unlink(paste0(rd,tile,".tif"),recursive=TRUE)
    }
  ...
  out <- list(
    ...
    alpha = as.numeric(coefficients(velocity)[['alpha']]),
    k = as.numeric(coefficients(velocity)[['k']]),
    tau_vmax = tau_vmax,
    tau_nlrq = tau_nlrq,
    data = data
  )
...
#' @param dem A character string or a RasterLayer object. Ignored unless the
...
makeWorld <- function(tiles,polys,tile_id,cut_slope,z_fix,
  ...
  if (directions == 4 | directions == 8){
    ...
    contiguity <- cieling(max(nrow(directions),ncol(directions)))
  }


  rd <- paste0(dir,"\\Tensors\\")
  for (i in tiles){
    if (!file.exists(paste0(rd,i,"_Tensor.gz"))){
      ...
      poly_buff <- gBuffer(poly,byid=TRUE,width=neighbor_distance)
      mzip <- intersect(poly_buff,polys)@data[,paste0(tile_id,".2")]

      getMap(mzip, polys = polys, tile_id = tile_id, dem = dem,dir = dir)

      mzip <- paste0(dir,"\\Elevations\\",mzip,".zip")
      lapply(mzip,unzip,exdir=tensors,junkpaths=TRUE)
...
regionMask <- function(region, z_fix){
  if (class(region) %in% c("SpatialPolygons","SpatialPolygonsDataFrame")){
    region <- fasterize(st_as_sf(region), z_fix)
...
importWorld <- function(region, z_fix, polys, banned = NULL,
  ...
  tiles <- paste0(dir,"\\Tensors\\",tiles,"_Tensor.gz")
  ...
  pb <- txtProgressBar(max = length(tiles), style = 3)
...
calculateCosts <- function(world, method = 'kuo', m = NULL, v_max = NULL,
  ...
  if(class(world) == "SpatialPolygonsDataFrame"){
  ...
  ][, c("x_i","y_i") := transpose(str_split(from,","))
  ][, c("x_f","y_f") := transpose(str_split(to,","))
...
getCosts <- function(world, from, to = NULL, id = 'ID', z_fix = NULL, x = "x", y = "y",
  ...
  if (class(from) %in% c("SpatialPolygons","SpatialPolygonsDataFrame")){
    from <- gCentroid(from, byid = TRUE, id = id)
  }
  ...
  if (destination != "all"){
    if (!is.null(to)){
      if (class(to) %in% c("SpatialPolygons","SpatialPolygonsDataFrame")){
        ...
        to <- gCentroid(to, byid = TRUE, id = id)
      }
  ...
  for (cost in costs){
    ...
    if (cost == "time"){
      Graph <- graph_from_data_frame(world[,.(from,to,weight = dt)])
    }
    if (cost == "work"){
      Graph <- graph_from_data_frame(world[,.(from,to,weight = dW_l)])
    }
    if (cost == "energy"){
      Graph <- graph_from_data_frame(world[,.(from,to,weight = dE_l)])
    }
    ...
    for (d in direction){
      ...
      if (destination == 'pairs'){
        ...
        outList[[paste0(cost,"_",d)]] <- xtabs(value~.,Distances)
      ...
      if (destination == 'all'){
        ...
        if ("file" %in% output){
          dir.create(paste0(dir,"\\CostRasters\\"),showWarnings=FALSE)
          writeRaster(outStack,
                      filename=paste0(dir,"\\CostRasters\\",
                                      str_to_sentence(cost),"_",
                                      names(outStack),".tif"),
...
makeCorridor <- function(rasters = tempdir(), order, costs = "all"){
  ...
  for (cost in costs){
    ...
    if (class(rasters) == "list"){
      ...
      rd <- paste0(normalizePath(rasters),"\\CostRasters\\")

      from <- paste0(rd,str_to_sentence(cost),"_",starts,".tif")
      to <- paste0(rd,str_to_sentence(cost),"_",stops,".tif")
...
getPaths <- function(world, nodes, id = "ID", order = NULL, z_fix = NULL, x = "x",
  ...
  if (class(nodes) %in% c("SpatialPolygons","SpatialPolygonsDataFrame")){
    nodes <- gCentroid(nodes, byid = TRUE, id = id)
  }
  # Coerce to data.table
  ...
  for (cost in costs){
    ...
    for (i in origins){
      ...
      legs <- shortest_paths(Graph,from=nodes[ID == i,Cell],
    ...
    lineList <- lapply(lineList,as,"SpatialLines")
