...
calculateCosts <- function(tiles = NULL, costFUN = energyCosts, dir = tempdir(),
  ...
  do_water <- FALSE
  if (file.exists(normalizePath(paste0(dir,"/water.gpkg")))){
    water <- vect(normalizePath(paste0(dir,"/water.gpkg")))
    do_water <- TRUE
  }
  ...
  for (i in tiles){
    ...
    if (do_water & doTile){
      if (length(intersect(grid[which(grid[['id']] == i), ], water)) > 0){
        ...
        water <- rasterize(water,z_fix,field=water_speed)
