...
`lbmech` is a geospatial package for `R` for least-cost path analysis in R.
...
for (1) in-place modification of objects greatly increasing processing speed,
(2) bidirectional raster analysis describing accumulated costs to and from
a node, and (3) additive and nonlinear transformations of large rasters without
running into integer overflow limits. `lbmech` also modularizes important aspects 
of the cost-distance workflow allowing for computationally-intensive and 
otherwise prohibitively-large operations to take place. Moreover, unlike similar 
tools such as `enerscape` for `R`, `lbmech` allows for the estimation of various 
...
those representing discrete landcover types), `dz` should be omitted. If
the origin x or y coordinates are needed, the `xy1` parameter in `importWorld`
...
sharing in common the `$from` and `$to` columns.Let's pretend that this raster represents landtypes,
and for some reason travel between cells of the same landcover type are prohibited.
Bearing in mind that since `distance = 16` a traveler could 'hop' over a single
barrier cell, we can still apply that condition using a simple data.table 
expression:
...
Let's say that the cost to travel from one landcover type to another is square of the
landcover class integer times the destination class integer divided by the distance
between the cells `dl`:
```{r makeCosts}
...
plot(paths,add=TRUE)
```
```{r finalCosts-display, echo = FALSE}
plot(corridors)
plot(paths,add=TRUE)
```
