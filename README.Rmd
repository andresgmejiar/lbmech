...
`lbmech` stores data and performs all linear algebra using the `data.table` package,
storing each possible movement as its own unique row, with entries for a from
node, a to node, and either the difference in or raw final and initial values of
the raster encountered during the transition. This allows for (1) in-place 
...
at scales on the order of 1-10 m. 
...
be between 1-10 m, while for ants it might be between 1-5 mm. 
