...
**PLEASE NOTE THAT THE README GUIDE IS ONLY ACCURATE AS TO VERSION 0.2.0.**
**VERSION 0.3.0 SWITCHED TO terra DEPENDENCIES FROM raster, AND I/O OPERATIONS
TO fst, BUT OPERATIONS ARE STILL ANALOGOUS.**

**VERSIONS >= 0.4.0 INTRODUCE A CONTAINED ENVIRONMENT FOR ALL OPERATIONS AS
OPPOSED TO REFERENCING A CONSTANT Z_FIX. THEREFORE THEY ARE ARE NOT BACKWARDS 
COMPATIBLE WITH THE PRESENT README, WHICH SHALL BE UPDATED SOON. HOWEVER,
ESTENSIVE DOCUMENTATION IS PROVIDED IN THE FUNCTION EXAMPLES**
...
simplifies the syntax necessary for many types of algebraically simple operations. 
`lbmech` stored each possible movement as its own unique row, with entries for a `from`
...
the cost-distance workflow. This allows for otherwise prohibitively-large 
...
object. You can often find these from state agencies.
    * As of the most recent version, `lbmech` supports file types 
    readable by `rgdal` and `raster`, as well as such files compressed in `gz` 
...
$\alpha$ the ideal slope of maximum walking speed, and $k$ a parameter controlling
...
estimated from locational data for different species. GPS data from GPX files
can be imported using the `importGPX` function. Data should be structured 
...
# 10,000 observations falling within the extent of the generated DEM
...
`v_lim` parameters to which dimensionless slope and velocity values are outlines
...
of constant direction of travel:
```{r print-matrix}
print(costMatrix)
```
...
a cost raster gives the absolute minimum expected total cost that would be required 
...
In the event of a tie in mosaiking rasters, we'll select the highest integer value 
