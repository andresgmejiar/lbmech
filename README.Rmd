...
see the provided vignettes. For most purposes, only five or six function calls
...
$$\frac{d\ell}{dt} = v_{\mathrm{max}} e^{-k | \frac{dz}{d\ell} - \alpha |},$$
where $\ell$ is horizontal displacement, $t$ time, $\frac{d\ell}{dt}$ horizontal
speed, $v_{\mathrm{max}}$ the maximum walking speed, $\frac{dz}{d\ell}$ the 
...
structure anyways:
```{r velocity, results = 'asis', echo = F}
print(velocity)
```
...
```{r raster-structure results = 'asis', echo = F}
...
```{r raster-dir, results = 'asis', echo = F}
list.files(normalizePath(paste0(dir,"/CostRasters")))
```

These rasters---be they stored as an object on the memory with `'object' %in% output`
or on the hard drive with `'file' %in% object'` are required to compute cost 
